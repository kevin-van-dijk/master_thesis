...
def mask_tokens(inputs, inputs_pad_mask, tokenizer, args):
    ...
    labels[~(masked_indices & inputs_pad_mask.bool())] = -100  # We only compute loss on masked non-pad tokens
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    train_dataloader = DataLoader(
        train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, collate_fn=pad_collate
    )
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    eval_dataloader = DataLoader(
        eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, collate_fn=pad_collate
    )
        ...
        for idx, batch in tqdm(enumerate(extract_dataloader), desc="Extracting", total=len(extract_dataloader)):
            ...
            with torch.no_grad():
                ...
                if args.model_type == "bert":
                    outputs = model.bert(text_ids, attention_mask=text_ids_mask)
                    all_layer_hidden_states = outputs[2]
                elif args.model_type == "roberta":
                    outputs = model.roberta(text_ids, attention_mask=text_ids_mask)
                    all_layer_hidden_states = outputs[2]
                elif args.model_type == "albert":
                    outputs = model.albert(text_ids, attention_mask=text_ids_mask)
                    all_layer_hidden_states = outputs[2]
                elif args.model_type == "electra":
                    outputs = model.electra(text_ids, attention_mask=text_ids_mask)
...
def main():
    ...
    if args.do_train and args.model_type in ["bert", "roberta", "distilbert", "camembert"] and not args.mlm:
        raise ValueError(
    ...
    if args.tokenizer_name:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, do_lower_case=args.do_lower_case, cache_dir=args.cache_dir)
    else:
        raise ValueError(
